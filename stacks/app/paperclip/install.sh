...
# Seed the paperclip-side hermes config. The hermes_local plugin spawns
# `hermes chat` inside this container, and the subprocess resolves provider
# + model from $HOME/.hermes/config.yaml (here: /paperclip/.hermes/config.yaml).
# Without this file the subprocess errors with "No models provided" on first
# wake. We render a minimal config with the three built-in providers — the
# api_key fields read at runtime from the container env (already populated
# by the operator via the install wizard / Portainer).
#
# Custom providers (Z.AI sub, Groq, etc.) are NOT seeded here: they belong
# to the hermes daemon's own dashboard config, not to paperclip. Operators
# wanting them locally can edit /paperclip/.hermes/config.yaml directly
...
rendered=$(
    ...
    envsubst <<'YAML'
# Rendered by bento/stacks/app/paperclip/install.sh — minimal hermes config
# for the hermes_local subprocess. Custom providers (Z.AI sub, Groq, etc.)
# are NOT seeded here: they belong to the hermes daemon's dashboard. To
# enable one locally, edit this file inside the container.
openai:
...
sudo docker exec -i -u node "$cid" sh -c 'mkdir -p /paperclip/.hermes && cat > /paperclip/.hermes/config.yaml && chmod 600 /paperclip/.hermes/config.yaml' <<< "$rendered"
echo "[paperclip] /paperclip/.hermes/config.yaml seeded"
